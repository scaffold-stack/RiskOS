...
function payload(hash: string, rollback: string[] = []) {
  const makeBlock = (value: string) => ({
    ...
    transactions: [{
      ...
      metadata: { success: true, receipt: { events: [{ type: "contract_event", data: { contract_identifier: "ST123.protocol", topic: "changed" } }] } },
    }],
  });
  return { chainhook: { uuid: "postgres-test" }, rollback: rollback.map(makeBlock), apply: [makeBlock(hash)] };
}
...
describe("PostgreSQL canonical ingestion", () => {
  beforeAll(async () => {
    ...
    if (database !== "riskos_integration") throw new Error("Refusing to clean a database not named riskos_integration");
  ...
  it("commits idempotently and atomically replaces a reorged block", async () => {
    ...
    expect(await store.ingest(first)).toMatchObject({ duplicate: false, appliedBlocks: 1, checkpointHeight: 501 });
    expect(await store.ingest(first)).toMatchObject({ duplicate: true, appliedBlocks: 0 });
    const conflictingDelivery = parseChainhookPayload(payload("conflict"), { network: "testnet", deliveryId: "pg-1" });
    ...
    expect(await store.ingest(reorg)).toMatchObject({ duplicate: false, appliedBlocks: 1, rolledBackBlocks: 1, invalidatedSnapshots: 1 });
    expect(await store.canonicalBlock("testnet", 501)).toMatchObject({ indexBlockHash: "index-b", canonical: true });
    const [oldBlock] = await sql`SELECT canonical FROM chain_blocks WHERE index_block_hash = 'index-a'`;
    const [oldSnapshot] = await sql`SELECT invalidated_at FROM position_snapshots WHERE snapshot_id = 'snapshot-pg-a'`;
  ...
  it("activates only a correctly signed registry from a trusted key", async () => {
    ...
    const activated = await registries.activate(signed, new Set([publicKeyFingerprint(publicKeyPem)]), new Date("2026-09-04"));
    ...
    await expect(registries.activate(altered, new Set([publicKeyFingerprint(publicKeyPem)]), new Date("2026-09-04"))).rejects.toThrow(/immutable/);
  });
  ...
  it("persists a Chainhook delivery through the real HTTP boundary", async () => {
    ...
    try {
      ...
      expect(await store.canonicalTip("testnet")).toMatchObject({ height: 502, indexBlockHash: "index-http-502" });
    } finally {
      await app.close();
    }
  });
  ...
  it("projects a registry-approved event and persists a hash-bound snapshot", async () => {
    ...
    const manifest: RegistryManifest = {
      version: "2026-09-04.2", network: "testnet", issuedAt: "2026-09-04T00:00:00.000Z", expiresAt: "2027-09-04T00:00:00.000Z",
      entries: [{ protocol: "zest-v2", adapterVersion: "projection-v1", network: "testnet", contractPrincipal: contract,
        interfaceHash: `sha256:${"a".repeat(64)}`, activationBlock: 1, supportedAssets: [], assetDefinitions: [], readOnlyFunctions: [], transactionFunctions: [],
        evidenceUrls: ["https://example.com/contract", "https://example.com/deployment"], enabled: true }],
    };
    const value = serializeCV(Cl.tuple({ action: Cl.stringAscii("collateral-add"), caller: Cl.standardPrincipal(address), data: Cl.tuple({
      account: Cl.standardPrincipal(address), amount: Cl.uint(10), "asset-id": Cl.uint(5), "updated-collateral-amount": Cl.uint(10),
    }) }));
    const batch = parseChainhookPayload({ chainhook: { uuid: "projection-test" }, rollback: [], apply: [{
      block_identifier: { index: 503, hash: "block-projection-503" }, metadata: { index_block_hash: "index-projection-503", burn_block_height: 966_853 },
      transactions: [{ transaction_identifier: { hash: "tx-projection-503" }, metadata: { success: true, receipt: { events: [{
        type: "SmartContractEvent", data: { contract_identifier: contract, topic: "print", hex_value: value },
      }] } } }],
    }] }, { network: "testnet", deliveryId: "projection-503" });
    await store.ingest(batch, manifest);
    expect(await store.protocolEventsForAddress("testnet", "zest", address)).toMatchObject([{ kind: "collateral-add", payload: { amountAtomic: "10" } }]);

    const position = {
      id: `zest:${address}`, type: "lending" as const, protocol: { id: "zest", version: "projection-v1", contract },
      collateral: { asset: "sBTC", amountAtomic: "10", decimals: 8, valueUsd: null }, debt: { asset: "USDCx", amountAtomic: "0", decimals: 6, valueUsd: null },
      parameters: { liquidationThresholdBps: 8000, maximumLtvBps: 7000 },
      provenance: [{ source: "chainhook" as const, blockHeight: 503, transactionId: "tx-projection-503", observedAt: "2026-09-04T00:00:00.000Z" }],
      confidence: { state: "verified" as const, score: 1, reasons: [] },
    };
    expect(await store.savePositionSnapshots({ network: "testnet", address, indexBlockHash: "index-projection-503", blockHeight: 503, registryVersion: manifest.version, positions: [position] })).toBe(1);
    expect(await store.currentPositionSnapshots("testnet", address)).toMatchObject([{ indexBlockHash: "index-projection-503", registryVersion: manifest.version, position: { id: position.id } }]);
    await expect(store.savePositionSnapshots({ network: "testnet", address, indexBlockHash: "not-canonical", blockHeight: 503, registryVersion: manifest.version, positions: [position] })).rejects.toThrow(/not canonical/);
  });
});
