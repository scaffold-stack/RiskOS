...
export class PostgresDataFoundationStore implements DataFoundationStore {
  ...
  async ingest(batch: ChainhookBatch, manifest: RegistryManifest | null = null): Promise<IngestionResult> {
    return this.sql.begin(async (tx) => {
      ...
      if (inserted.length === 0) {
        const existing =
          await tx`SELECT payload_sha256 FROM raw_chain_events WHERE event_key = ${batch.eventKey}`;
        ...
        const checkpoint =
          await tx`SELECT last_height FROM ingestion_checkpoints WHERE source_id = ${batch.source} AND network = ${batch.network}`;
        return {
          duplicate: true,
          appliedBlocks: 0,
          rolledBackBlocks: 0,
          invalidatedSnapshots: 0,
          checkpointHeight: checkpoint[0] ? Number(checkpoint[0].last_height) : null,
        };
      }

      let invalidatedSnapshots = 0;
      for (const block of batch.rollback)
        invalidatedSnapshots += await this.rollbackBlock(tx, batch.network, block.indexBlockHash);
      for (const block of batch.apply)
        invalidatedSnapshots += await this.applyBlock(tx, batch, block, manifest);
  ...
  private async rollbackBlock(
    ...
    const invalidated =
      await sql`UPDATE position_snapshots SET invalidated_at = now() WHERE network = ${network} AND index_block_hash = ${indexBlockHash} AND invalidated_at IS NULL RETURNING snapshot_id`;
  ...
  private async applyBlock(
    ...
    const competing =
      await sql`SELECT index_block_hash FROM chain_blocks WHERE network = ${batch.network} AND height = ${block.height} AND canonical AND index_block_hash <> ${block.indexBlockHash}`;
    let invalidated = 0;
    for (const row of competing)
      invalidated += await this.rollbackBlock(sql, batch.network, String(row.index_block_hash));
    await sql`
  ...
  async canonicalBlock(network: "mainnet" | "testnet", height: number): Promise<CanonicalBlock | null> {
    const rows = await this
      .sql`SELECT index_block_hash, height, canonical, burn_block_height FROM chain_blocks WHERE network = ${network} AND height = ${height} AND canonical LIMIT 1`;
    const row = rows[0];
    return row
      ? {
          network,
          indexBlockHash: String(row.index_block_hash),
  ...
  async canonicalTip(network: "mainnet" | "testnet"): Promise<CanonicalBlock | null> {
    const rows = await this
      .sql`SELECT index_block_hash, height, canonical, burn_block_height FROM chain_blocks WHERE network = ${network} AND canonical ORDER BY height DESC LIMIT 1`;
    const row = rows[0];
    return row
      ? {
          network,
  ...
  async sourceHealth() {
    const rows = await this
      .sql`SELECT source_id, state, observed_height, lag_blocks, detail FROM source_health ORDER BY source_id`;
    return rows.map((row) => ({
  ...
  async savePositionSnapshots(input: PositionSnapshotBatch): Promise<number> {
    ...
    return this.sql.begin(async (tx) => {
      const block =
        await tx`SELECT height FROM chain_blocks WHERE network = ${input.network} AND index_block_hash = ${input.indexBlockHash} AND canonical FOR SHARE`;
      if (!block[0] || Number(block[0].height) !== input.blockHeight)
      ...
      for (const position of positions) {
        ...
        const rows = await tx`
          INSERT INTO position_snapshots ${tx({
            ...
            index_block_hash: input.indexBlockHash,
            block_height: input.blockHeight,
            position: tx.json(position as never),
            lineage: tx.json({
              registryVersion: input.registryVersion,
  ...
  async currentPositionSnapshots(
    ...
    return rows.map((row) => ({
      ...
      registryVersion: String((row.lineage as { registryVersion?: unknown }).registryVersion ?? "unknown"),
  ...
  async positionSnapshotHistory(
    ...
    // `limit` is observation-oriented (canonical blocks), not raw position rows.
    // Selecting the newest N blocks first avoids truncating a multi-position tip.
    const observationLimit = Math.max(1, limit);
    const rows = await this.sql`
      WITH recent_observations AS (
        SELECT DISTINCT block_height, index_block_hash
        FROM position_snapshots
        WHERE network = ${network} AND address = ${address} AND invalidated_at IS NULL
      ...
      latest_per_position AS (
        SELECT DISTINCT ON (snapshot.index_block_hash, snapshot.position_id)
          snapshot.snapshot_id, snapshot.index_block_hash, snapshot.block_height,
          snapshot.position_id, snapshot.position, snapshot.lineage, snapshot.created_at
        FROM position_snapshots snapshot
        INNER JOIN recent_observations recent
          ON recent.block_height = snapshot.block_height
          AND recent.index_block_hash = snapshot.index_block_hash
        WHERE snapshot.network = ${network}
          AND snapshot.address = ${address}
          AND snapshot.invalidated_at IS NULL
        ORDER BY snapshot.index_block_hash, snapshot.position_id, snapshot.created_at DESC
      )
      SELECT snapshot_id, index_block_hash, block_height, position, lineage
      FROM latest_per_position
      ORDER BY block_height DESC, position_id
    `;
    return rows.map((row) => ({
      snapshotId: String(row.snapshot_id),
      network,
  ...
  async cashFlowEventsForAddress(
    ...
    const rows = await this.sql`
      ...
      WHERE network = ${network}
        ...
        AND canonical
        AND protocol IN ('zest', 'bitflow')
        AND kind = ANY(${kindList})
    ...
    return rows
      .map((row) => ({
        ...
        txId: String(row.tx_id),
        eventIndex: Number(row.event_index),
        protocol: String(row.protocol) as "zest" | "bitflow" | "sbtc",
        adapterVersion: String(row.adapter_version),
        kind: String(row.kind),
        ownerAddress: row.owner_address === null ? null : String(row.owner_address),
        positionKey: String(row.position_key),
