...
export interface DataFoundationStore {
  ...
  currentPositionSnapshots(network: "mainnet" | "testnet", address: string): Promise<StoredPositionSnapshot[]>;
  protocolEventsForAddress(network: "mainnet" | "testnet", protocol: "zest" | "bitflow" | "sbtc", address: string): Promise<ProtocolProjection[]>;
}
...
export class MemoryDataFoundationStore implements DataFoundationStore {
  ...
  readonly snapshots = new Map<string, { indexBlockHash: string; invalidated: boolean; value?: StoredPositionSnapshot }>();
  readonly projections = new Map<string, ProtocolProjection & { canonical: boolean }>();
  readonly health = new Map<string, { state: "green" | "amber" | "red"; observedHeight: number | null; lagBlocks: number | null; detail: string | null }>();
  ...
  async ingest(batch: ChainhookBatch, manifest: RegistryManifest | null = null): Promise<IngestionResult> {
    ...
    if (existingDigest) {
      return { duplicate: true, appliedBlocks: 0, rolledBackBlocks: 0, invalidatedSnapshots: 0, checkpointHeight: this.checkpointHeight };
    }
    this.rawEvents.set(batch.eventKey, batch.payloadSha256);
    this.health.set(batch.source, { state: "green", observedHeight: batch.apply.at(-1)?.height ?? null, lagBlocks: 0, detail: "Latest Chainhook delivery processed" });
    ...
    for (const rollback of batch.rollback) {
      ...
      for (const projection of this.projections.values()) if (projection.indexBlockHash === rollback.indexBlockHash) projection.canonical = false;
    ...
    for (const applied of batch.apply) {
      for (const existing of this.blocks.values()) {
        if (existing.network === batch.network && existing.height === applied.height && existing.indexBlockHash !== applied.indexBlockHash) {
          existing.canonical = false;
          for (const projection of this.projections.values()) if (projection.indexBlockHash === existing.indexBlockHash) projection.canonical = false;
          for (const snapshot of this.snapshots.values()) {
      ...
      for (const projection of projected.projections) this.projections.set(projection.projectionId, { ...projection, canonical: true });
  ...
  async canonicalBlock(network: "mainnet" | "testnet", height: number): Promise<CanonicalBlock | null> {
    return [...this.blocks.values()].find((block) => block.network === network && block.height === height && block.canonical) ?? null;
  }
  ...
  async updateSourceHealth(input: { sourceId: string; state: "green" | "amber" | "red"; observedHeight: number | null; lagBlocks: number | null; detail: string | null }): Promise<void> {
  ...
  async savePositionSnapshots(input: PositionSnapshotBatch): Promise<number> {
    ...
    if (!block?.canonical || block.height !== input.blockHeight) throw new Error("Snapshot block is not canonical at the declared height");
    let inserted = 0;
    for (const position of input.positions) {
      ...
      this.snapshots.set(snapshotId, { indexBlockHash: input.indexBlockHash, invalidated: false, value: {
        snapshotId, network: input.network, address: input.address, indexBlockHash: input.indexBlockHash,
        blockHeight: input.blockHeight, registryVersion: input.registryVersion, position,
      } });
    }
    return inserted;
  }
  ...
  async currentPositionSnapshots(network: "mainnet" | "testnet", address: string): Promise<StoredPositionSnapshot[]> {
    ...
    for (const snapshot of this.snapshots.values()) {
      if (snapshot.invalidated || !snapshot.value || snapshot.value.network !== network || snapshot.value.address !== address) continue;
      const existing = latest.get(snapshot.value.position.id);
      if (!existing || existing.blockHeight < snapshot.value.blockHeight) latest.set(snapshot.value.position.id, snapshot.value);
    }
  ...
  async protocolEventsForAddress(network: "mainnet" | "testnet", protocol: "zest" | "bitflow" | "sbtc", address: string): Promise<ProtocolProjection[]> {
    const keys = new Set([...this.projections.values()].filter((event) => event.canonical && event.network === network && event.protocol === protocol && event.ownerAddress === address).map((event) => event.positionKey));
    return [...this.projections.values()].filter((event) => event.canonical && event.network === network && event.protocol === protocol && keys.has(event.positionKey));
  }
}
